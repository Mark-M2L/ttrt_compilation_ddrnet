...
class DAPPM(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x_list[4].shape)

        # print(torch.cat(x_list, 1).shape)

        # print(self.shortcut(x).shape)
       
        # out = self.compression(torch.cat(x_list, 1)) + self.shortcut(x)

        # print(f"forward dappm shape: {out.shape}")
...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # For tensorRT (did not exist originally):
        else:
            temp = x_
...
def DualResNet_imagenet(cfg, pretrained=False):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=2, planes=32, spp_planes=128, head_planes=64, augment=True)
    if pretrained:
...
if __name__ == '__main__':
    x = torch.rand(2, 3, 600, 400)
    net = DualResNet_imagenet(None)
    y = net(x)
    print(y.shape)